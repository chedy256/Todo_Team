...
class LocalDatabaseService {
  ...
  // Add methods for task synchronization with API
  Future<void> syncTasksFromApi(List<Task> apiTasks) async {
    final db = await database;

    // Clear existing tasks
    await db.delete(_tasksTableName);

    // Insert tasks from API
    for (final task in apiTasks) {
      await db.insert(
        _tasksTableName,
        {
          _tasksIdColumn: task.id,
          _tasksTitleColumn: task.title,
          _tasksDescriptionColumn: task.description,
          _tasksPriorityColumn: task.priority.index,
          _tasksOwnerIdColumn: task.ownerId,
  ...
  Future<bool> hasTasks() async {
    final db = await database;
    final result = await db.rawQuery('SELECT COUNT(*) as count FROM $_tasksTableName');
